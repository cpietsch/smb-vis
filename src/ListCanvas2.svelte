<script>
    ...
    $: res = transition === 3 ? "1024" : "256";
    $: loading = transition === 3 ? "lazy" : "eager";
    ...
    function animateToCloud() {
        route.set({ ...$route, transition: "list-cloud" });
        transition = 2;
    }
    ...
    $: {
        ...
        if (loadedAll && $route.transition == "list-cloud") {
            domBoxes = getDomBoxes();
            console.log("domBoxes", domBoxes);
            canvasBoxes = getCanvasBoxes();
            console.log("canvasBoxes", canvasBoxes);

            setTimeout(() => (transition = 1), 100);
            // setTimeout(() => (transition = 0), 2100);
        }
                    ...
                    <div class="metacontainer">
                        <div class="meta">
                            ...
                            <div class="additional">
                                ...
                                <p>
                                    <b>Sammlung</b><span>{item.data.sammlunglong}</span>
                                </p>
                                {#each fields as field}
                                    {#if item.data[field[0]] !== ''}
                                        <p>
                                            <b>{field[1]}</b><span>{item.data[field[0]]}</span>
                                        </p>
                                    {/if}
                                {/each}
                                <p>
